...
## 도메인 규칙
- `type`: `deposit`(입금) | `withdraw`(출금) | `exchange_to_krw`(메소→원 환전) | `exchange_to_meso`(원→메소 환전)
...
- `tag`: 선택, 20자 이하 (메모 대체 — 예: 보스, 사냥, 큐브). 환전 폼은 자동으로 "환전"
...
6. **입력 폼**: 입금/출금 토글, 메소/원 토글, 금액(메소는 억 단위 소수), 태그(최근 태그 칩 + 자유 입력), 일시, 저장
7. **거래 목록**: 날짜별 그룹핑. 입금(초록 +)/출금(빨강 −)/환전(남색 "3.5억 메소 → ₩6,300"),
