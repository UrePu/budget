...
/** 환전 거래에 자동으로 붙는 태그 — 선택 칩에는 노출하지 않는다 (자동 부여 전용) */
export const EXCHANGE_TAG = "환전";

/** 전체 프리셋 (중복 제거, 환전 제외 — 환전 기록이 있으면 달력 필터에 데이터 기반으로 뜸) */
export const ALL_PRESET_TAGS: string[] = Array.from(
  new Set([...PRESET_TAGS.meso, ...PRESET_TAGS.krw]),
);
